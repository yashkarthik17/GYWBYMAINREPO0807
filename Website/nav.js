...
(function () {
  ...
  var css = [
    ...
    ".gnav-mini i{display:block;height:2px;margin:2px 0;border-radius:2px;background:#FFF9EE;transition:transform .2s ease,opacity .2s ease;}",
    ...
    "  background:rgb(18,27,52);border:1px solid rgba(255,255,255,.14);box-shadow:0 18px 40px rgba(0,0,0,.45);",
    ...
    // menu open: the glass bar turns SOLID so the dropdown reads as one
    // opaque sheet instead of floating over the video (user direction)
    ".gnav--overlay.gnav--open{background:rgb(18,27,52);border-bottom-color:rgba(230,196,106,.35);box-shadow:0 4px 18px rgba(0,0,0,.35);}",
    ...
    ".gnav__burger{flex:none;display:flex;flex-direction:column;justify-content:center;width:42px;height:42px;",
    "  padding:0 10px;border:1px solid rgba(29,43,80,.22);border-radius:12px;background:transparent;cursor:pointer;}",
    ".gnav__burger i{display:block;height:2px;margin:2px 0;border-radius:2px;background:#1D2B50;transition:transform .2s ease,opacity .2s ease;}",
  ...
  function build() {
    ...
    function setOpen(open) {
      burger.setAttribute("aria-expanded", open ? "true" : "false");
      panel.classList.toggle("open", open);
      bar.classList.toggle("gnav--open", open);
    }
    burger.addEventListener("click", function () {
      setOpen(burger.getAttribute("aria-expanded") !== "true");
    });
