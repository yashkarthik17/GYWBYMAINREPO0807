    ...
    if (JOURNEY) {
      if (jv && jReady && !jv.seeking) {
        ...
        if (span) {
          ...
          const diff = tt - jv.currentTime;
          jCur = tt;   // (kept for the debug readout)
          if (diff > 10) {
            // forward teleport (nav-dot jump, huge fling): one seek. Ordinary lag
            // never seeks — a mid-flight skip reads as a stutter; trailing smoothly
            // and catching up reads as cinematic.
            try { jv.pause(); } catch (e) {}
            try { jv.currentTime = tt; } catch (e) {}
          } else if (diff < -0.06) {
            ...
            // guard above throttles to decoder speed). Tight 4-frame GOP keeps each
            // seek cheap, so this is the smoothest reverse the device can do.
            if (!jv.paused) { try { jv.pause(); } catch (e) {} }
            try { jv.currentTime = tt; } catch (e) {}
          } else if (diff > 0.06) {
            // chase forward by playing (muted, so >2x is safe; decode is the limit)
            const rate = Math.min(2.5, Math.max(0.5, diff * 1.5));
            if (jv.playbackRate !== rate) { try { jv.playbackRate = rate; } catch (e) {} }
            if (jv.paused && !jPlayPending) jPlay();
          } else if (!jv.paused) {
            try { jv.pause(); } catch (e) {}   // caught up: settle on the frame
          }
        }
      }
