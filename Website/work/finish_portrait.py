"""Portrait chain post-processing: trim-align seams, encode as the mobile tier, posters, SSIM gate."""
...
def enc_mobile(src, out, start=0.0, end_t=None):
    # portrait mobile encode: 720 wide, tight GOP for phone seeking
...
# 1) dives: tail-trim to the handoff frame; encode as <name>-m.mp4 (the phone tier)
for n in NAMES:
    d = dur(f"pdive_{n}.mp4")
    enc_mobile(f"pdive_{n}.mp4", os.path.join(V, f"{n}-m.mp4"), 0.0, d - TAIL)
    print(f"portrait dive {n} -> {n}-m.mp4 (dur {d - TAIL:.2f})", flush=True)

# 2) connectors: sweep tail vs next dive's first frame, trim where it helps; encode conn<i>-m.mp4
...
# 3) posters from the new portrait mobile encodes
for n in NAMES:
    png = "_pposter.png"
...
print("portrait posters extracted", flush=True)

# 4) SSIM gate over the portrait chain
chain = ["beige-m.mp4", "conn1-m.mp4", "crashpad-m.mp4", "conn2-m.mp4", "crash-m.mp4",
         "conn3-m.mp4", "glowup-m.mp4", "conn4-m.mp4", "finale-m.mp4"]
print("=== PORTRAIT SSIM SEAM GATE ===", flush=True)
worst = 1.0
