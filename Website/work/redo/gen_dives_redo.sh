#!/bin/bash
# Redo portrait chain, phase 1: 5 dives (9:16, seedance_2_0) + boundary frame extraction.
...
for n in beige crashpad crash glowup finale; do
  if [ -s "pdive_$n.mp4" ]; then
    ...
    echo "OK   pdive_$n.mp4 $(du -h "pdive_$n.mp4" | cut -f1)"
  else
    echo "MISS pdive_$n.mp4"
  fi
done
